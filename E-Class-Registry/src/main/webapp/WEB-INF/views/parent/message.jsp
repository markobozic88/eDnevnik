...
<html>
    ...
    <body>
        ...
        <div class="messs">
            <c:forEach items="${allMessagesBetweenParentAndTeacher}" var="message">
                ...
                <dl class="sender${message.getSenderStatus()}">
                    ...
                    <c:if test = "${message.getSenderStatus() == 3}">
                    <dt class="ss">${message.getTeacher().getFirstName()} ${message.getTeacher().getFirstName()}:</dt>
                    </c:if>
                    <c:if test = "${message.getSenderStatus() == 4}">
                    <dt class="ss">You:</dt>
                    </c:if>
                    <dt class="ss"><kbd>${message.getFormatedDate()}</kbd></dt>
                    
                    <b><dt class="ss">${message.getMessage()}</dt></b><br><br>
        ...
        <div class="text">
            <form:form action="/e-register/parent/sendmessage" method="post" modelAttribute="messageDto">
                <form:hidden path="communicationId" />
                <form:hidden path="parentId" />
                <form:hidden path="teacherId" />
                <form:hidden path="senderStatus"/>
                <form:textarea path="message" rows="20" cols="100" placeholder="Enter new message"></form:textarea>
